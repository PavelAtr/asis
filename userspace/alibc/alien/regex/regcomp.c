...
int regcomp(regex_t *preg, const char *pattern, int cflags) {
  ...
  g = (struct re_guts *) malloc(sizeof(struct re_guts) + (NC - 1) * sizeof
(cat_t));
...
static void p_ere_exp(struct parse *p) {
  ...
  if (!( c == '*' || c == '+' || c == '?' || (c == '{' && MORE2() && isdigit
(PEEK2())))) return; // No repetition, we're done
  NEXT();
...
static cset *allocset(struct parse *p) {
  ...
  if (no >= p->ncsalloc) {
    ...
    if (p->g->setbits == NULL) {
      ...
      for (i = 0; i < no; i++) p->g->sets[i].ptr = p->g->setbits + css * (i /
CHAR_BIT);
    }
...
static void doinsert(struct parse *p, sop op, size_t opnd, sopno pos) {
  ...
  memmove(&p->strip[pos + 1], &p->strip[pos], (HERE() - pos - 1) * sizeof
(sop));
  p->strip[pos] = s;
}
